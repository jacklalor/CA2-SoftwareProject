        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse($sleeping as $sleeping)
                <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-md sm:rounded-lg">
                    <img src="{{ asset('storage/' . $sleeping->image_url) }}" alt="{{ $sleeping->name }}">
                    <div class="p-6">
                        ...
                        @if($sleeping->seller && $sleeping->seller->name) <!-- Check if seller relationship exists and seller's name is not null or empty -->
                            <p class="text-gray-700 dark:text-gray-300 mt-2">Listed By: {{ $sleeping->seller->name }}</p> <!-- Display "Listed By:" before seller's name -->
                        @else
            ...
            @empty
                <p class="text-gray-600 dark:text-gray-400">No sleepings available.</p>
            @endforelse
        </div>
    </div>
