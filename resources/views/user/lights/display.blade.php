...
@section('content')
    <div class="flex justify-center pt-8 pb-16"> <!-- Added padding top and bottom -->
        <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            @forelse($lights as $lights)
                <div class="bg-white dark:bg-gray-900 overflow-hidden shadow-md sm:rounded-lg">
                    <img src="{{ asset('storage/' . $lights->image_url) }}" alt="{{ $lights->name }}">
                    <div class="p-6">
                        <h3 class="text-lg font-semibold text-gray-800 dark:text-gray-200">{{ $lights->name }}</h3>
                        <p class="text-gray-600 dark:text-gray-400">{{ $lights->sub_description }}</p>
                        @if($lights->seller && $lights->seller->name) <!-- Check if seller relationship exists and seller's name is not null or empty -->
                            <p class="text-gray-700 dark:text-gray-300 mt-2">Listed By: {{ $lights->seller->name }}</p> <!-- Display "Listed By:" before seller's name -->
                        @else
                            <p class="text-gray-700 dark:text-gray-300 mt-2">Seller not provided</p> <!-- Fallback message -->
                        @endif

                        <p class="text-gray-900 dark:text-white text-xl font-bold mt-2">€ {{ $lights->price }}</p>
                        <button class="bg-orange-500 hover:bg-orange-600 text-white font-bold py-2 px-4 rounded mt-4">Buy</button> <!-- Added button -->
                    </div>
                </div>
            @empty
                <p class="text-gray-600 dark:text-gray-400">No lightss available.</p>
            @endforelse
        </div>
    </div>
